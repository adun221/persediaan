...
session_start();
 include '../konek.php'; // untuk mengambil koneksi ke db
		...
		$queri = mysqli_query($koneksi,"DELETE from nama_barang where id='".$_GET['id']."'"); // query Hapus data ke tabel 

		if ($queri) {
			// echo mysqli_error($koneksi);
		    header("Location: http://localhost/persediaan/view/main/index.php?h=nama_barang&del=0"); // otomatis mengalihkan ke halaman
		} else {
			// echo mysqli_error($koneksi);
		    header("Location: http://localhost/persediaan/view/main/index.php?h=nama_barang&del=1"); // otomatis mengalihkan ke halaman
		}
